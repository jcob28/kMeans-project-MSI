...
# Wczytanie wyników
evaluator_scores = np.load('../results/exp3/exp3_scores.npy')
...
for m, metryka in enumerate(metryki):
    fold_size = 50
    plt.figure(figsize=(32, 20))
    plt.plot(np.mean(evaluator_scores[0, :, m].reshape(10, -1), axis=1), label="IncrKmeans")
    plt.plot(np.mean(evaluator_scores[1, :, m].reshape(10, -1), axis=1), label="MiniBatchKMeans")
    plt.xlabel("Numer iteracji", fontsize='26')
    plt.ylabel(f"{metryka.__name__}", fontsize='26')
    plt.legend(fontsize='26')
    plt.gca().xaxis.set_tick_params(labelsize=24)
    plt.gca().yaxis.set_tick_params(labelsize=24)
    plt.gca().grid(True, linestyle='--', linewidth=1)
    plt.savefig(f"../results/exp3/exp3_mean_{metryka.__name__}.png")
    plt.show()
...
for m, metryka in enumerate(metryki):
    ...
    for i in range(10):
        ...
        formatted_score = "{:.3f}({:d})".format(mean_score, int(std_score * 100))
